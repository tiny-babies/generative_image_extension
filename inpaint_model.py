...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x, mask, reuse=False,
        ...
        with tf.compat.v1.variable_scope(name, reuse=reuse), \
            ...
            # stage1
            x = gen_conv(x, cnum, 5, 1, name='conv1', training=False)
            x = gen_conv(x, 2*cnum, 3, 2,
                         name='conv2_downsample', training=False)
            x = gen_conv(x, 2*cnum, 3, 1, name='conv3', training=False)
            x = gen_conv(x, 4*cnum, 3, 2,
                         name='conv4_downsample', training=False)
            x = gen_conv(x, 4*cnum, 3, 1, name='conv5', training=False)
            x = gen_conv(x, 4*cnum, 3, 1, name='conv6', training=False)
            mask_s = resize_mask_like(mask, x)
            x = gen_conv(x, 4*cnum, 3, rate=2,
                         name='conv7_atrous', training=False)
            x = gen_conv(x, 4*cnum, 3, rate=4, name='conv8_atrous', training=False)
            x = gen_conv(x, 4*cnum, 3, rate=8, name='conv9_atrous', training=False)
            x = gen_conv(x, 4*cnum, 3, rate=16,
                         name='conv10_atrous', training=False)
            x = gen_conv(x, 4*cnum, 3, 1, name='conv11', training=False)
            x = gen_conv(x, 4*cnum, 3, 1, name='conv12', training=False)
            x = gen_deconv(x, 2*cnum, name='conv13_upsample', training=False)
            x = gen_conv(x, 2*cnum, 3, 1, name='conv14', training=False)
            x = gen_deconv(x, cnum, name='conv15_upsample', training=False)
            x = gen_conv(x, cnum//2, 3, 1, name='conv16', training=False)
            x = gen_conv(x, 3, 3, 1, activation=None,
                         name='conv17', training=False)
            x = tf.nn.tanh(x)
            x_stage1 = x
